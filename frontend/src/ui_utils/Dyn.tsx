...
export function Dyn({children}: {children: React.ReactNode}) {
  const [dyn_vals, set_dyn_vals] = ui.useStateWithUpdate({} as Record<string, {val: number, lo: number, hi: number}>)
  const [visible, set_visible] = React.useState(true)
  const q = React.useRef({} as typeof dyn_vals)
  const components =
    <div style={{position: 'fixed', right: 0, bottom: 0, width: 280, background: '#fff8', padding: '0 15px', border: '1px #333a solid', borderRadius: 5,
    ...
    >
    {Object.entries(dyn_vals).sort(utils.by(([k]) => k)).map(([k, {val, lo, hi}]) =>
      <div style={{height: 58}}>
        <label style={{position: 'relative', top: 12, zIndex: 1000, color: '#444'}}>{k + ':'}</label>
        <Slider
          style={{zIndex:999}}
          min={lo}
          max={hi}
          ...
          valueLabelFormat={(s: number) => ('' + utils.roundDown(s, 3)).replace(/\..*/, s => s.slice(0, 3))}
        />
      </div>
    )}
    </div>
  ...
  return <DynCtx.Provider value={dyn}>
    {components}
    {children}
  </DynCtx.Provider>
}
